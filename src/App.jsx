...
const defaultBlock = (type) => {
  ...
  if (type === "image") return { id: uid(), type, src: null, caption: "", showHearts: false };
  if (type === "poll") return { id: uid(), type, question: "Quelle est votre réponse ?", questionFontSize: 20, options: [{ id: uid(), label: "Option A", image: null }, { id: uid(), label: "Option B", image: null }], correctOptionId: null, color: "#6366f1" };
  return null;
};
...
function stripBase64(slides) {
  return slides.map(sl => ({
    ...sl,
    bentoPhotos: (sl.bentoPhotos || []).map(p => ({ ...p, src: p.src?.startsWith("data:") ? null : p.src })),
    columns: (sl.columns || []).map(col => ({
      ...col,
      blocks: (col.blocks || []).map(b => b.type === "image" && b.src?.startsWith("data:") ? { ...b, src: null } : b),
    })),
  }));
}
function safeSave(key, value) {
  try { localStorage.setItem(key, JSON.stringify(value)); return true; }
  catch { try { localStorage.removeItem("sb_autosave"); localStorage.setItem(key, JSON.stringify(value)); return true; } catch { return false; } }
}

// ─── BENTO LAYOUT ENGINE ──────────────────────────────────────────────────────
function buildBentoLayout(n) {
  if (n === 0) return { cols: "1fr", rows: "1fr", cells: [] };
  const TEMPLATES = {
    1: { cols: "1fr", rows: "1fr", cells: [{ col:"1/2", row:"1/2" }] },
    2: { cols: "3fr 2fr", rows: "1fr", cells: [{ col:"1/2", row:"1/2" }, { col:"2/3", row:"1/2" }] },
    3: { cols: "2fr 1fr", rows: "1fr 1fr", cells: [{ col:"1/2", row:"1/3" }, { col:"2/3", row:"1/2" }, { col:"2/3", row:"2/3" }] },
    4: { cols: "2fr 1fr 1fr", rows: "1fr 1fr", cells: [{ col:"1/2", row:"1/3" }, { col:"2/3", row:"1/2" }, { col:"3/4", row:"1/2" }, { col:"2/4", row:"2/3" }] },
    5: { cols: "2fr 1fr 1fr", rows: "1fr 1fr", cells: [{ col:"1/2", row:"1/3" }, { col:"2/3", row:"1/2" }, { col:"3/4", row:"1/2" }, { col:"2/3", row:"2/3" }, { col:"3/4", row:"2/3" }] },
    6: { cols: "1fr 2fr 1fr", rows: "1fr 1fr", cells: [{ col:"1/2", row:"1/2" }, { col:"2/3", row:"1/3" }, { col:"3/4", row:"1/2" }, { col:"1/2", row:"2/3" }, { col:"3/4", row:"2/3" }, { col:"2/3", row:"2/3" }] },
    7: { cols: "1fr 1fr 2fr", rows: "1fr 1fr 1fr", cells: [{ col:"1/2", row:"1/2" }, { col:"2/3", row:"1/2" }, { col:"3/4", row:"1/3" }, { col:"1/2", row:"2/3" }, { col:"2/3", row:"2/3" }, { col:"1/3", row:"3/4" }, { col:"3/4", row:"3/4" }] },
    8: { cols: "1fr 2fr 1fr", rows: "1fr 1fr 1fr", cells: [{ col:"1/2", row:"1/2" }, { col:"2/3", row:"1/3" }, { col:"3/4", row:"1/2" }, { col:"1/2", row:"2/3" }, { col:"3/4", row:"2/3" }, { col:"1/2", row:"3/4" }, { col:"2/3", row:"3/4" }, { col:"3/4", row:"3/4" }] },
    9: { cols: "1fr 1fr 1fr", rows: "2fr 1fr 1fr", cells: [{ col:"1/2", row:"1/2" }, { col:"2/3", row:"1/2" }, { col:"3/4", row:"1/2" }, { col:"1/2", row:"2/3" }, { col:"2/4", row:"2/3" }, { col:"1/3", row:"3/4" }, { col:"3/4", row:"3/4" }, { col:"2/3", row:"3/4" }, { col:"1/2", row:"2/3" }] },
  };
  if (n <= 9 && TEMPLATES[n]) return TEMPLATES[n];
  const NCOLS = 3;
  const occupied = {};
  const mark = (c, r, w, h) => { for (let dc = 0; dc < w; dc++) for (let dr = 0; dr < h; dr++) occupied[`${c+dc},${r+dr}`] = true; };
  const isFree = (c, r, w, h) => { for (let dc = 0; dc < w; dc++) for (let dr = 0; dr < h; dr++) if (occupied[`${c+dc},${r+dr}`]) return false; return true; };
  const findFree = (w, h) => { for (let r = 1; r <= 100; r++) for (let c = 1; c <= NCOLS - w + 1; c++) if (isFree(c, r, w, h)) return { c, r }; return { c: 1, r: 1 }; };
  const cells = [];
  let maxRow = 1;
  for (let i = 0; i < n; i++) {
    const isWide = (i % 5 === 0) && (i > 0 || n > 3);
    const w = isWide ? 2 : 1;
    const { c, r } = findFree(w, 1);
    mark(c, r, w, 1);
    cells.push({ col: `${c}/${c+w}`, row: `${r}/${r+1}` });
    if (r > maxRow) maxRow = r;
  }
  return { cols: `repeat(${NCOLS}, 1fr)`, rows: `repeat(${maxRow}, 1fr)`, cells };
}
        ...
        <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, right: 0, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 10, boxShadow: "0 8px 32px rgba(0,0,0,0.14)", zIndex: 999, maxHeight: 240, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.map(f => { loadGoogleFont(f); return (
              <div key={f} onClick={() => { onChange(f); loadGoogleFont(f); setOpen(false); setSearch(""); }} style={{ padding: "8px 12px", cursor: "pointer", fontSize: 14, fontFamily: f, background: f === current ? "#f5f3ff" : "transparent", color: f === current ? "#6366f1" : "#1e1b4b", fontWeight: f === current ? 700 : 400 }}>{f}</div>
            ); })}
          </div>
        </div>
      )}
...
function LottieHearts() {
  ...
  useEffect(() => {
    const init = () => {
      ...
      animRef.current = window.lottie.loadAnimation({ container: containerRef.current, renderer: "svg", loop: true, autoplay: true, animationData: HEARTS_LOTTIE });
    };
    if (window.lottie) { init(); }
    else {
      const existing = document.getElementById("lottie-cdn");
...
async function uploadImageToStorage(file, onProgress) {
  ...
  onProgress(10);
  let fakeProgress = 10;
  const progressInterval = setInterval(() => { fakeProgress = Math.min(fakeProgress + 8, 85); onProgress(fakeProgress); }, 200);
  try {
    const { data, error } = await supabase.storage.from("slide-images").upload(path, file, { cacheControl: "3600", upsert: false, contentType: file.type });
  ...
  } catch (err) { clearInterval(progressInterval); onProgress(0); return { url: null, error: err }; }
}
...
function PollDisplay({ block, theme, voteCounts = {}, totalVotes = 0, showCorrect = false, myVoteId = null, revealed = false }) {
  ...
  const GAP = 16;
  const LABEL_TO_BAR = 4;
  const questionH = qFontSize * 1.4 + 12;
  ...
  useEffect(() => {
    if (showCorrect && !prevShowCorrect.current && hasImages && block.correctOptionId) { setRevealPhase(1); setTimeout(() => setRevealPhase(2), 600); }
  ...
  if (revealPhase === 2 && hasImages && correctOpt) {
    return (
      <div ref={containerRef} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, animation: "fadeIn 0.5s ease" }}>
        ...
        {correctOpt.image && <div style={{ width: 180, height: 180, borderRadius: "50%", overflow: "hidden", border: "5px solid #10b981", boxShadow: "0 0 0 8px rgba(16,185,129,0.15)", animation: "popIn 0.5s cubic-bezier(0.34,1.56,0.64,1)" }}><img src={correctOpt.image} alt={correctOpt.label} style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>}
...
function BlockRenderer({ block, theme, scale = 1, fill = false, voteCounts, totalVotes, showCorrect, myVoteId, revealed }) {
  ...
  if (block.type === "image") return block.src
    ? <div style={{ ...(fill ? { position: "absolute", inset: 0 } : { flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }), alignItems: "center", justifyContent: "center", position: "relative" }}>
        <img src={block.src} alt={block.caption || ""} style={{ display: "block", ...(fill ? { position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "contain" } : { width: "100%", height: "100%", objectFit: "contain", borderRadius: 10 * scale, flex: 1, minHeight: 0 }) }} />
        {block.showHearts && <LottieHearts />}
...
function BentoCanvas({ slide, theme, scale = 1 }) {
  ...
  return (
    <div style={{ background: theme?.bgColor || "#fff", height: "100%", display: "flex", flexDirection: "column", padding: pad, gap: Math.max(6, 10 * scale), boxSizing: "border-box", overflow: "hidden" }}>
      {slide.title && !slide.hideTitle && <p style={{ margin: 0, fontSize: titleFs, fontWeight: 800, textAlign: "center", color: theme?.textColor || "#1e1b4b", fontFamily: font, flexShrink: 0, lineHeight: 1.2 }}>{slide.title}</p>}
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: cols, gridTemplateRows: frRows, gap, minHeight: 0 }}>
        {photos.map((photo, i) => {
          const cell = cells[i]; if (!cell) return null;
          return (
            <div key={photo.id} style={{ gridColumn: cell.col, gridRow: cell.row, overflow: "hidden", borderRadius: radius, background: "#e5e7eb", position: "relative" }}>
              {photo.src && <img src={photo.src} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", display: "block" }} />}
            </div>
          );
        })}
      </div>
...
function SlideCanvas({ slide, theme, scale = 1, voteCounts = {}, totalVotes = 0, showCorrect = false, myVoteId = null, revealed = false }) {
  ...
  return (
    <div style={{ background: theme?.bgColor || "#fff", height: "100%", display: "flex", flexDirection: "column", padding: 24 * scale, boxSizing: "border-box", fontFamily: font, overflow: "hidden" }}>
      {slide.title && !slide.hideTitle && <p style={{ margin: "0 0 14px", fontSize: 11 * scale, fontWeight: 700, color: theme?.accentColor || "#818cf8", textTransform: "uppercase", letterSpacing: "0.1em", fontFamily: font, flexShrink: 0 }}>{slide.title}</p>}
      <div style={{ display: "flex", gap: 20 * scale, flex: 1, overflow: "hidden", minHeight: 0 }}>
        {slide.columns.map((col, i) => {
          ...
          const imgBlock = imageOnly ? col.blocks[0] : null;
          if (imageOnly) return (
            <div key={col.id} style={{ flex, minWidth: 0, position: "relative", borderRadius: 12 * scale, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", background: theme?.bgColor || "#fff" }}>
              {imgBlock?.src ? <img src={imgBlock.src} alt="" style={{ width: "100%", height: "100%", objectFit: "contain", display: "block" }} /> : <div style={{ width: "100%", height: "100%", background: "#f3f4f6", display: "flex", alignItems: "center", justifyContent: "center", color: "#9ca3af", fontSize: 14 }}>📷</div>}
              {imgBlock?.showHearts && <LottieHearts />}
            </div>
          );
          return (
            <div key={col.id} style={{ flex, minWidth: 0, display: "flex", flexDirection: "column", gap: 14 * scale, overflow: "hidden", height: "100%" }}>
              {col.blocks.map(block => (
                <div key={block.id} style={{ flex: (block.type === "poll" || block.type === "image") ? 1 : "none", flexShrink: (block.type === "poll" || block.type === "image") ? 1 : 0, minHeight: 0, display: "flex", flexDirection: "column" }}>
                  <BlockRenderer block={block} theme={theme} scale={scale} fill={false} voteCounts={block.type === "poll" ? voteCounts : {}} totalVotes={block.type === "poll" ? totalVotes : 0} showCorrect={showCorrect} revealed={revealed} />
                </div>
              ))}
            </div>
          );
...
function ImageBlockEditor({ block, onChange, onDelete, onUploadStart, onUploadEnd }) {
  ...
  const handleFile = async (file) => {
    if (!file) return;
    setUploading(true); setProgress(5); onUploadStart?.();
    const { url, error } = await uploadImageToStorage(file, (p) => setProgress(p));
    setUploading(false);
    if (error) { const r = new FileReader(); r.onload = (ev) => { onChange({ src: ev.target.result }); setProgress(100); onUploadEnd?.(); }; r.readAsDataURL(file); }
    else { onChange({ src: url }); setProgress(100); onUploadEnd?.(); }
  ...
  return (
    <div style={s.blockEditor}>
      ...
      {!uploading && progress === 100 && <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 10px", background: "#d1fae5", borderRadius: 8, marginBottom: 6 }}><span style={{ color: "#10b981", fontWeight: 700, fontSize: 12 }}>✓ Image enregistrée</span></div>}
      {block.src && !uploading
        ? <div style={{ position: "relative" }}><img src={block.src} alt="" style={{ width: "100%", borderRadius: 8, maxHeight: 110, objectFit: "cover", display: "block" }} /><button onClick={() => onChange({ src: null })} style={{ position: "absolute", top: 5, right: 5, background: "rgba(0,0,0,0.6)", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", padding: "1px 6px", fontSize: 11 }}>✕</button></div>
        : !uploading ? <button onClick={() => fileRef.current.click()} style={s.uploadBtn}>+ Choisir une image</button> : null}
      {block.src && <input value={block.caption} onChange={e => onChange({ caption: e.target.value })} placeholder="Légende (optionnel)" style={{ ...s.input, marginTop: 7, width: "100%", boxSizing: "border-box" }} />}
      {block.src && (
        <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, cursor: "pointer", padding: "8px 10px", background: block.showHearts ? "#fff0f6" : "#f9fafb", borderRadius: 8, border: `1px solid ${block.showHearts ? "#f9a8d4" : "#e5e7eb"}` }}>
          <input type="checkbox" checked={!!block.showHearts} onChange={e => onChange({ showHearts: e.target.checked })} style={{ cursor: "pointer", width: 16, height: 16 }} />
          <span style={{ fontSize: 12, fontWeight: 700, color: block.showHearts ? "#db2777" : "#6b7280" }}>💕 Animation coeurs en overlay</span>
        </label>
      )}
    </div>
  );
...
function PollBlockEditor({ block, theme, onChange, onDelete, onUploadStart, onUploadEnd }) {
  ...
  const handleOptImage = async (optId, file) => {
    if (!file) return;
    setOptUploading(prev => ({ ...prev, [optId]: true })); setOptProgress(prev => ({ ...prev, [optId]: 5 })); onUploadStart?.();
    const { url, error } = await uploadImageToStorage(file, (p) => setOptProgress(prev => ({ ...prev, [optId]: p })));
    onUploadEnd?.(); setOptUploading(prev => ({ ...prev, [optId]: false }));
    if (!error && url) { onChange({ options: block.options.map(o => o.id === optId ? { ...o, image: url } : o) }); }
    else { const r = new FileReader(); r.onload = ev => onChange({ options: block.options.map(o => o.id === optId ? { ...o, image: ev.target.result } : o) }); r.readAsDataURL(file); }
  ...
  return (
    <div style={s.blockEditor}>
      ...
      <label style={{ ...s.label, marginBottom: 10 }}>Taille de la question<input type="number" value={block.questionFontSize || 20} min={12} max={60} onChange={e => onChange({ questionFontSize: Number(e.target.value) })} style={{ ...s.input, width: 72, marginTop: 4 }} /></label>
      ...
      {block.options.map((opt, i) => (
        <div key={opt.id} style={{ marginBottom: 10, background: "#f9fafb", borderRadius: 10, padding: "8px 10px" }}>
          <div style={{ display: "flex", gap: 6, alignItems: "center", marginBottom: 6 }}>
            ...
            <button onClick={() => onChange({ correctOptionId: block.correctOptionId === opt.id ? null : opt.id })} title="Bonne réponse" style={{ width: 26, height: 26, borderRadius: "50%", border: "2px solid", borderColor: block.correctOptionId === opt.id ? "#10b981" : "#d1d5db", background: block.correctOptionId === opt.id ? "#10b981" : "transparent", color: block.correctOptionId === opt.id ? "#fff" : "#9ca3af", cursor: "pointer", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>✓</button>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <input ref={el => fileRefs.current[opt.id] = el} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { handleOptImage(opt.id, e.target.files[0]); e.target.value = ""; }} />
            {opt.image ? (
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", overflow: "hidden", border: "2px solid #e0e7ff", flexShrink: 0 }}><img src={opt.image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /></div>
            ...
            ) : optUploading[opt.id] ? <UploadProgress progress={optProgress[opt.id] || 0} label="Upload…" />
            : <button onClick={() => fileRefs.current[opt.id]?.click()} style={{ fontSize: 11, color: "#818cf8", background: "#f0eeff", border: "1px dashed #c4b5fd", borderRadius: 99, padding: "3px 10px", cursor: "pointer", fontWeight: 600 }}>+ Photo ronde</button>}
          </div>
        </div>
      ))}
...
function BlockEditor({ block, theme, onChange, onDelete, onUploadStart, onUploadEnd }) {
  ...
  if (block.type === "image") return <ImageBlockEditor block={block} onChange={onChange} onDelete={onDelete} onUploadStart={onUploadStart} onUploadEnd={onUploadEnd} />;
  if (block.type === "poll") return <PollBlockEditor block={block} theme={theme} onChange={onChange} onDelete={onDelete} onUploadStart={onUploadStart} onUploadEnd={onUploadEnd} />;
}
...
function ColumnEditor({ column, colIndex, theme, onChange, activeBlockId, setActiveBlockId, onUploadStart, onUploadEnd }) {
  ...
  const moveBlock = (blockId, dir) => { const blocks = [...column.blocks]; const i = blocks.findIndex(b => b.id === blockId); if (i + dir < 0 || i + dir >= blocks.length) return; [blocks[i], blocks[i + dir]] = [blocks[i + dir], blocks[i]]; onChange({ blocks }); };
  return (
    <div style={{ borderLeft: colIndex > 0 ? "2px dashed #e0e7ff" : "none", paddingLeft: colIndex > 0 ? 12 : 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}><span style={{ fontSize: 10, fontWeight: 700, color: "#818cf8", textTransform: "uppercase", letterSpacing: "0.06em" }}>Col. {colIndex + 1}</span></div>
      ...
      {column.blocks.map((block, i) => (
        <div key={block.id} onClick={() => setActiveBlockId(block.id)} style={{ ...s.blockRow, ...(block.id === activeBlockId ? s.blockRowActive : {}), margin: "0 0 5px" }}>
...
function BentoEditor({ slide, onUpdateSlide, onUploadStart, onUploadEnd }) {
  ...
  const addPhotos = async (files) => {
    ...
    const results = await Promise.all(fileArr.map(async (file, i) => {
      ...
      const { url, error } = await uploadImageToStorage(file, (p) => setUploadProgresses(prev => ({ ...prev, [tempId]: p })));
      setUploadProgresses(prev => { const n = { ...prev }; delete n[tempId]; return n; });
      if (error || !url) { return new Promise(res => { const r = new FileReader(); r.onload = e => res({ id: uid(), src: e.target.result }); r.readAsDataURL(file); }); }
  ...
  const movePhoto = (id, dir) => { const arr = [...photos]; const i = arr.findIndex(p => p.id === id); if (i + dir < 0 || i + dir >= arr.length) return; [arr[i], arr[i + dir]] = [arr[i + dir], arr[i]]; onUpdateSlide({ bentoPhotos: arr }); };
  ...
  return (
    <div>
      ...
      {photos.length === 0 && pending.length === 0 && <div style={{ ...s.uploadBtn, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", height: 80, flexDirection: "column", gap: 6 }} onClick={() => fileRef.current.click()}><span style={{ fontSize: 22 }}>📷</span><span>Importer des photos</span></div>}
    ...
    <div>
      ...
      <div style={{ marginTop: 16, borderRadius: 10, overflow: "hidden", border: "1px solid #e0e7ff" }}>
        <div style={{ background: theme.bgColor, padding: 14 }}>
          ...
          <div style={{ height: 7, borderRadius: 4, background: theme.pollBarBg, overflow: "hidden" }}><div style={{ height: "100%", width: "65%", background: theme.accentColor, borderRadius: 4 }} /></div>
        </div>
      </div>
    </div>
  );
...
function SlideThumbnail({ slide, index, isActive, onClick, onDelete, onDuplicate, onDragStart, onDragOver, onDrop, isDragOver }) {
  ...
  const blockCount = slide.layout === "bento" ? (slide.bentoPhotos?.length || 0) + " 📷" : (slide.columns?.flatMap(c => c.blocks).length || 0) + " blocs";
  return (
    <div draggable onDragStart={onDragStart} onDragOver={e => { e.preventDefault(); onDragOver(); }} onDrop={onDrop} onClick={onClick}
...
function PresentationMode({ slides, theme, sessionCode, sessionId, onExit }) {
  ...
  const syncReveal = useCallback(async (correct) => { if (!sessionId) return; await supabase.from("sessions").update({ reveal_results: true, reveal_correct: correct }).eq("id", sessionId); }, [sessionId]);
  useEffect(() => {
    if (!sessionId) return;
  ...
  useEffect(() => { const h = (e) => { if (e.key === "ArrowRight" || e.key === " ") go(1); if (e.key === "ArrowLeft") go(-1); }; window.addEventListener("keydown", h); return () => window.removeEventListener("keydown", h); }, [currentIdx, slides.length]);
  return (
      ...
      {!barHidden && (
        <div style={{ background: "#1e1b4b", padding: "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #312e81", flexShrink: 0, gap: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
            {pollBlock && <button onClick={handleRevealCorrect} style={{ ...s.presBtn, background: revealCorrect ? "#10b981" : "rgba(16,185,129,0.15)", borderColor: "#10b981", color: revealCorrect ? "#fff" : "#10b981" }}>✓ {revealCorrect ? "Réponse visible" : "Révéler réponse"}</button>}
            ...
            <button onClick={() => setBarHidden(true)} style={{ background: "none", border: "none", color: "#6b7280", cursor: "pointer", fontSize: 11, fontWeight: 700, padding: "4px 6px", textDecoration: "underline" }}>Masquer la barre</button>
          </div>
        </div>
      )}
      {barHidden && (
        <div style={{ position: "absolute", top: 10, right: 14, zIndex: 50, display: "flex", alignItems: "center", gap: 8 }}>
          {pollBlock && <button onClick={handleRevealCorrect} style={{ ...s.presBtn, background: revealCorrect ? "#10b981" : "rgba(30,27,75,0.75)", borderColor: "#10b981", color: revealCorrect ? "#fff" : "#10b981", backdropFilter: "blur(4px)" }}>✓ {revealCorrect ? "Réponse visible" : "Réponse"}</button>}
          ...
          <button onClick={() => setBarHidden(false)} style={{ background: "rgba(30,27,75,0.75)", backdropFilter: "blur(4px)", border: "1px solid rgba(129,140,248,0.3)", color: "#a5b4fc", cursor: "pointer", fontSize: 11, fontWeight: 700, padding: "7px 12px", borderRadius: 8 }}>Afficher la barre</button>
        </div>
      )}
...
function ParticipantView({ joinCode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: slidesData } = await supabase.from("slides").select("id, position, title, blocks, layout, columns, bento_photos, hide_title").eq("presentation_id", sess.presentation_id).order("position", { ascending: true });
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`part-${sessionData.sessionId}`)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "sessions", filter: `id=eq.${sessionData.sessionId}` }, (p) => {
        setCurrentIdx(p.new.current_slide_index || 0); setRevealResults(!!p.new.reveal_results); setRevealCorrect(!!p.new.reveal_correct);
      }).subscribe();
    return () => supabase.removeChannel(ch);
  }, [sessionData]);
  ...
  const normalizeSlide = (sl) => {
    if (!sl) return null;
    if (sl.columns) { const cols = parseJ(sl.columns); return { ...sl, hideTitle: !!sl.hide_title, layout: sl.layout || "single", columns: cols.map(c => ({ ...c, blocks: parseJ(c.blocks) })), bentoPhotos: parseJ(sl.bento_photos || "[]") }; }
              ...
              <div>
                ...
                {!hasVoted ? (
                  <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                    {poll.options.map(opt => <button key={opt.id} onClick={() => handleVote(opt.id)} style={{ ...s.voteBtn, background: theme?.bgColor || "#fff", color: theme?.textColor || "#1e1b4b", borderColor: (theme?.accentColor || "#6366f1") + "44", fontFamily: font }}>{opt.label}</button>)}
                  </div>
                ) : (
                  <div>
                    ...
                    {revealResults ? poll.options.map(opt => {
                      ...
                      return (
                        <div key={opt.id} style={{ marginBottom: 9 }}>
                          ...
                          <div style={{ background: theme?.pollBarBg || "#e5e7eb", borderRadius: 8, height: 9 }}><div style={{ height: "100%", borderRadius: 8, background: bg, width: isMyV ? "100%" : "0%", transition: "width 0.5s" }} /></div>
                        </div>
                      );
                    }) : (
...
function Dashboard({ onOpen }) {
  ...
  const load = async () => { setLoading(true); const { data } = await supabase.from("draft_presentations").select("id, title, updated_at").order("updated_at", { ascending: false }); setPresentations(data || []); setLoading(false); };
  useEffect(() => { load(); }, []);
  const handleCreate = async () => {
    if (!newName.trim()) return;
    setCreating(true);
    const { data, error } = await supabase.from("draft_presentations").insert({ title: newName.trim(), slides_data: JSON.stringify([defaultSlide()]), theme_data: JSON.stringify(defaultTheme()) }).select().single();
    setCreating(false);
    if (error) { alert("Erreur : " + error.message); return; }
    setShowNew(false); setNewName(""); onOpen(data);
  };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif", minHeight: "100vh", background: "#f8f9ff" }}>
      <div style={{ background: "#1e1b4b", padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}><div style={s.logo}>◈</div><span style={{ fontWeight: 800, fontSize: 20, color: "#fff", letterSpacing: "-0.02em" }}>SlideBuilder</span></div>
        <button onClick={() => setShowNew(true)} style={{ background: "#6366f1", border: "none", color: "#fff", padding: "9px 22px", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 800, fontFamily: "inherit" }}>+ Nouvelle présentation</button>
      </div>
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "40px 24px" }}>
        ...
        {!loading && presentations.length === 0 && (
          <div style={{ textAlign: "center", padding: "80px 0", background: "#fff", borderRadius: 20, border: "2px dashed #e0e7ff" }}>
            ...
            <button onClick={() => setShowNew(true)} style={{ background: "#6366f1", border: "none", color: "#fff", padding: "12px 28px", borderRadius: 10, cursor: "pointer", fontSize: 15, fontWeight: 800, fontFamily: "inherit" }}>+ Nouvelle présentation</button>
          </div>
        )}
        {!loading && presentations.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 16 }}>
            {presentations.map(p => (
              <div key={p.id} onClick={() => onOpen(p)} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e0e7ff", overflow: "hidden", boxShadow: "0 2px 12px rgba(99,102,241,0.06)", cursor: "pointer" }}
                ...
                <div style={{ height: 130, background: "linear-gradient(135deg, #ede9fe 0%, #dbeafe 100%)", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontSize: 40, opacity: 0.25 }}>◈</span></div>
                <div style={{ padding: "14px 16px 16px" }}>
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={e => { e.stopPropagation(); onOpen(p); }} style={{ flex: 1, background: "#6366f1", border: "none", color: "#fff", padding: "8px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: "inherit" }}>Ouvrir →</button>
                    <button onClick={e => handleDelete(e, p.id)} disabled={deleting === p.id} style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#ef4444", padding: "8px 12px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700, opacity: deleting === p.id ? 0.5 : 1 }}>{deleting === p.id ? "…" : "✕"}</button>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showNew && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(30,27,75,0.55)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setShowNew(false)}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: 420, maxWidth: "95vw" }} onClick={e => e.stopPropagation()}>
            ...
            <input autoFocus value={newName} onChange={e => setNewName(e.target.value)} onKeyDown={e => e.key === "Enter" && newName.trim() && handleCreate()} placeholder="Ex : Quiz Marketing Q2" style={{ ...s.input, width: "100%", boxSizing: "border-box", fontSize: 16, marginBottom: 20, padding: "10px 14px" }} />
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button onClick={handleCreate} disabled={!newName.trim() || creating} style={{ flex: 2, background: "#6366f1", border: "none", color: "#fff", padding: "11px", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 800, fontFamily: "inherit", opacity: !newName.trim() || creating ? 0.5 : 1 }}>{creating ? "Création…" : "Créer"}</button>
            </div>
          </div>
        </div>
      )}
...
function Builder({ draftId, initialTitle, onBack }) {
  ...
  const saveToSupabase = useCallback((sl, th, ttl) => {
    ...
    saveTimeout.current = setTimeout(async () => {
      const { error } = await supabase.from("draft_presentations").update({ title: ttl, slides_data: JSON.stringify(stripBase64(sl)), theme_data: JSON.stringify(th), updated_at: new Date().toISOString() }).eq("id", draftId);
      setSaveStatus(error ? "error" : "saved");
    }, 1500);
  }, [draftId]);
  useEffect(() => { if (draftId && slides.length) saveToSupabase(slides, theme, title); }, [slides, theme, title]);
  const normalizeSlide = useCallback((sl) => { if (sl.columns) return sl; return { ...sl, layout: "single", columns: [{ id: uid(), blocks: sl.blocks || [] }] }; }, []);
  const activeSlide = slides.find(ss => ss.id === activeSlideId) || slides[0];
  const normActive = normalizeSlide(activeSlide);
  const updateSlide = useCallback((id, patch) => setSlides(prev => prev.map(ss => ss.id === id ? { ...ss, ...patch } : ss)), []);
  const updateColumn = useCallback((slideId, colId, patch) => setSlides(prev => prev.map(ss => ss.id === slideId ? { ...ss, columns: ss.columns.map(c => c.id === colId ? { ...c, ...patch } : c) } : ss)), []);
  const setLayout = (layoutId) => {
    ...
    const columns = layout.cols === 1 ? [{ id: cur.columns[0]?.id || uid(), blocks: cur.columns.flatMap(c => c.blocks) }] : cur.columns.length >= 2 ? [cur.columns[0], cur.columns[1]] : [cur.columns[0] || { id: uid(), blocks: [] }, { id: uid(), blocks: [] }];
  ...
  const deleteSlide = (id) => { if (slides.length === 1) return; const r = slides.filter(ss => ss.id !== id); setSlides(r); if (activeSlideId === id) setActiveSlideId(r[0].id); };
  const duplicateSlide = (sl) => { const n = { ...JSON.parse(JSON.stringify(sl)), id: uid() }; setSlides(prev => { const i = prev.findIndex(ss => ss.id === sl.id); const next = [...prev]; next.splice(i + 1, 0, n); return next; }); setActiveSlideId(n.id); };
  const handleDrop = (dropIdx) => { if (dragIdx === null || dragIdx === dropIdx) { setDragIdx(null); setDragOverIdx(null); return; } const next = [...slides]; const [moved] = next.splice(dragIdx, 1); next.splice(dropIdx, 0, moved); setSlides(next); setDragIdx(null); setDragOverIdx(null); };
  const handlePresent = async () => {
    ...
    try {
      ...
      for (let i = 0; i < slides.length; i++) {
        ...
        await supabase.from("slides").insert({ presentation_id: pres.id, position: i, title: sl.title, layout: sl.layout, hide_title: !!sl.hideTitle, columns: JSON.stringify(sl.columns || []), bento_photos: JSON.stringify(sl.bentoPhotos || []), blocks: "[]" });
      }
  ...
  return (
    <div style={s.root}>
      <div style={s.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 11, fontWeight: 700, color: saveStatus === "error" ? "#ef4444" : saveStatus === "saving" ? "#9ca3af" : "#10b981" }}>{saveStatus === "saving" ? "💾 Enregistrement…" : saveStatus === "error" ? "⚠ Erreur" : "✓ Sauvegardé"}</span>
          <button onClick={handlePresent} disabled={uploadsInProgress > 0} style={{ ...s.presentBtn, ...(uploadsInProgress > 0 ? { opacity: 0.5, cursor: "not-allowed", background: "#9ca3af" } : {}) }}>
      ...
      <div style={s.workspace}>
        ...
        <div style={s.editorCenter}>
          <div style={{ padding: "12px 18px 10px", borderBottom: "1px solid #f3f4f6", flexShrink: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <input value={normActive.title} onChange={e => updateSlide(activeSlide.id, { title: e.target.value })} style={{ ...s.slideTitleInput, flex: 1, opacity: normActive.hideTitle ? 0.35 : 1 }} placeholder="Titre de la slide" />
              <label style={{ display: "flex", alignItems: "center", gap: 5, cursor: "pointer", flexShrink: 0, fontSize: 11, fontWeight: 700, color: normActive.hideTitle ? "#6366f1" : "#9ca3af", whiteSpace: "nowrap" }}>
                <input type="checkbox" checked={!!normActive.hideTitle} onChange={e => updateSlide(activeSlide.id, { hideTitle: e.target.checked })} style={{ cursor: "pointer" }} />Masquer
              </label>
            </div>
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              ...
              {LAYOUTS.map(layout => (
                <button key={layout.id} onClick={() => setLayout(layout.id)} style={{ ...s.addBlockBtn, padding: "3px 9px", fontSize: 11, background: normActive.layout === layout.id ? "#ede9fe" : "#f9f9f9", borderColor: normActive.layout === layout.id ? "#818cf8" : "#e0e7ff", color: normActive.layout === layout.id ? "#6366f1" : "#6b7280", fontWeight: normActive.layout === layout.id ? 700 : 400 }}>
        ...
        <div style={s.previewPanel}>
          ...
          {rightPanel === "preview" && (
            <>
              <div style={{ ...s.slideCanvas, aspectRatio: "16/9", overflow: "hidden" }}><SlideCanvas slide={normActive} theme={theme} scale={0.6} /></div>
...
export default function App() {
  ...
  if (view === "builder" && currentDraft) return <Builder draftId={currentDraft.id} initialTitle={currentDraft.title} onBack={() => { setView("dashboard"); setCurrentDraft(null); }} />;
